<?php
  ...
  $name = mysqli_real_escape_string($mysql,$_POST["name"]);$name = ucfirst($name);
  ...
  if($countusname > 0)
  {
    echo'<p class="failure">Your username already exists.<p>';
  }
  elseif($countemail > 0)
  {
  ...
  else
  {
    ...
    mysqli_query($mysql,"INSERT INTO UsersBackup (Name,Username,Password,Email,Admin) VALUES('".$name."','".$usname."','".$password."','".$email."','$admin')") or die("Inserting the user has failed.");
    $password = crypt($password,$salt);
    mysqli_query($mysql,"INSERT INTO Users (Name,Username,Password,Email,Admin,Active,Hash) VALUES('".$name."','".$usname."','$password','".$email."','$admin','0','$hash')") or die("Inserting the user has failed.");
    ...
    echo'<p class="succes">The account has been made. Verify it by clicking the activation link that has been send to its email.</p>';
  }
